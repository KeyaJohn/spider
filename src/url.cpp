...
void push_surlqueue(Surl *url)
{
    ...
    {
        //SPIDER_LOG(SPIDER_LEVEL_DEBUG, "将URL:%s加入到surl_queue队列中..............", url->url);
...
void * urlparser(void *none)
{
    ...
    while(1) 
    {
        ...
        //printf("从队列surl_queue中取出一个url:%s用于解析DNS\n",url->url);
...
int extract_url(regex_t *re, char *str, Url *ourl)
{
    ...
    char *p = str;
    /*
    regexec()函数的使用方式：re编译后的正则表达式，p需要匹配的目标字符串，nmatch是保存
    匹配的个数，matchptr存储每个匹配的字符串的起始位置，其结构为:
    typedef struct 
    {
        regoff_t rm_so;
        regoff_t rm_eo;
    ...
    */
    while (regexec(re, p, nmatch, matchptr, 0) != REG_NOMATCH) 
    {
        ...
        if (url != NULL) 
        {
            ...
            //SPIDER_LOG(SPIDER_LEVEL_DEBUG, "normalize url 后的url: %s",surl->url);
            
            //判断是否爬取过该网页
            if (iscrawled(surl->url)) 
            { 
                //SPIDER_LOG(SPIDER_LEVEL_DEBUG, "已爬取过该网页: %s", surl->url);
                free(surl->url);
                free(surl);
                continue;
            } 
